"""Batch SAM3D 3D Reconstruction Script
    ...
    try:
        ...
        if process.returncode == 0:
            if os.path.exists(glb_path):
                ...
                print(f"⚠️  Process completed but GLB not found: {glb_path}")
                return False
        else:
...
def main():
    ...
    for obj_name in OBJECTS:
        ...
        if success:
            print(f"✅ {obj_name} completed successfully")
        else:
            print(f"❌ {obj_name} failed")
    ...
    if successful == total:
        ...
        print("⚠️  Some objects failed. Check individual logs for details.")
