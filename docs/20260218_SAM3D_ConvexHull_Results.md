...
Generated by `visualize_convex_hull_growth.py`. Each 4-panel shows:

- **Top-left:** Normal deviation angle map (RdYlGn_r, 0–60°); green outline = original mask boundary; red overlay = pixels exceeding threshold
...
- **Bottom-right:** Normal angle histogram (inside mask vs growth region, adaptive threshold line)

![ito_en_bottle_mask_growth](../output/sam3d_convex_hull_v2/vis/ito_en_bottle_mask_growth.png)

**ito_en_bottle:** +4,772px at shoulder/neck gaps (threshold=60.0°). Curved bottle surface handled by 60° max cap — captures the cylindrical body arc while stopping at background.

![envelope_mask_growth](../output/sam3d_convex_hull_v2/vis/envelope_mask_growth.png)

**envelope:** +727px filling corner gaps (threshold=29.0°). Flat rectangular shape, tight adaptive threshold correctly limits growth to the envelope surface.

![headphones_mask_growth](../output/sam3d_convex_hull_v2/vis/headphones_mask_growth.png)

**headphones:** Only +71px — mask was already nearly convex (threshold=60.0°, hull gap only 61px).

![alienware_keyboard_mask_growth](../output/sam3d_convex_hull_v2/vis/alienware_keyboard_mask_growth.png)

**alienware_keyboard:** +2,129px filling left-edge concavity (threshold=48.0°). Flat plate benefits from direct assignment — no connectivity barrier.

![green_tea_bottle_mask_growth](../output/sam3d_convex_hull_v2/vis/green_tea_bottle_mask_growth.png)

**green_tea_bottle:** Only +209px (threshold=18.3°) — the hull region is mostly background/air around the large shadow mask. Normal-consistency correctly rejects these pixels as having different surface normals. This is *better* than Sobel (+5,903px) which blindly added more flat shadow/table pixels.
