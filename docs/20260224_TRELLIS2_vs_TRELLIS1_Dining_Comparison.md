# TRELLIS2 vs TRELLIS1 (SAM3D) — Dining Scene Comparison
...
| **SAM segmentation** | 52.7s | reused from T1 run |
| **Total time** | 567.9s (9.5 min) | ~1186s (19.8 min) |
| **Avg IoU** | **0.53** | **0.23** |

**TRELLIS1 achieves 2.3x better alignment (IoU) in half the time.**
...
### TRELLIS1 Objects (sorted by IoU)
...
| Physical Object | T1 Name (IoU) | T2 Name (IoU) | Delta |
|:---|:---|:---|:---:|
| Patterned sofa | sofa_with_patterned_cover (0.25) | sofa_cover (0.40) | **T2 +0.15** |
| Table with cloth | table_with_flower_tablecloth (0.48) | tablecloth (0.20) | T1 +0.28 |
| Wooden chair | wooden_chair (0.23) | chair (0.24) | ~tied |
| Newspaper | newspaper (0.84) | newspaper (0.15) | T1 +0.69 |

**Only the sofa improved with TRELLIS2.** The newspaper shows the largest regression — T1 achieved 0.84 IoU vs T2's 0.15.
...
![TRELLIS1 Masks](../output/sam3d_dining_t1/viz/all_masks_grid.png)

**Note:** Different SAM runs produced different object decompositions. TRELLIS2 segmented: sofa_cover, tablecloth, chair_cover, chair, pillow_and_blanket, newspaper, pot_and_trivet, plant. TRELLIS1 segmented: sofa_with_patterned_cover, table_with_flower_tablecloth, broken_tile, wooden_chair, neck_pillow, newspaper, metal_colander, placemat.
...
### TRELLIS1 Scene Render

![T1 Comparison](../output/sam3d_dining_t1/full_scene_comparison.png)

**Left:** Original target | **Right:** TRELLIS1 3D render with MoGe camera alignment
...
| Property | TRELLIS1 | TRELLIS2 |
|:---|:---|:---|
| Visual type | Vertex colors (baked) | Vertex colors (from O-Voxel remesh) |
| UV textures | No (TRELLIS1 bakes to vertices) | PBR textures on GCP (`_pbr.glb`), vertex colors locally |
...
**Key finding:** TRELLIS2 GLBs are ~28x larger than TRELLIS1 due to much higher polygon counts from the O-Voxel decoder. However, higher polygon count does not translate to better alignment — vertex density != geometric accuracy.
...
TRELLIS1's pipeline includes a **Sparse Structure (SS) model** that predicts initial rotation, translation, and scale for each object. This gives ICP a good starting pose, leading to reliable convergence.
...
**Evidence:** Module 4b experiment (2026-02-24) showed that applying TRELLIS1's SS model to TRELLIS2 objects produces no improvement (avg IoU 0.157 → 0.157), confirming the SS model is trained end-to-end with TRELLIS1's decoder output space and doesn't transfer.

### 2. Mesh Topology Differences

TRELLIS2's Flexicubes decoder produces different surface topology than TRELLIS1's Gaussian splatting decoder. Even after remeshing to fix triangle soup, the surface normals and vertex distributions differ, making ICP registration harder.
...
| Metric | TRELLIS1 | TRELLIS2 | Winner |
|:---|:---:|:---:|:---:|
| Average IoU | **0.53** | 0.23 | TRELLIS1 (2.3x) |
| Best single object | **0.90** | 0.58 | TRELLIS1 |
| Objects with IoU > 0.5 | **4/8** | 1/8 | TRELLIS1 |
| Reconstruction time | **515s** | 805s | TRELLIS1 (1.6x faster) |
| Total pipeline time | **568s** | ~1186s | TRELLIS1 (2.1x faster) |
| GLB size (total) | **11 MB** | 311 MB | TRELLIS1 (28x smaller) |
| Scene render quality | Good overlap | Poor (holes, gaps) | TRELLIS1 |

**Conclusion:** TRELLIS1 (SAM3D) significantly outperforms TRELLIS2 on the dining scene benchmark across all metrics — alignment quality, speed, and output efficiency. The gap is primarily driven by TRELLIS1's SS pose prior, which TRELLIS2 lacks and cannot replicate via post-hoc methods. TRELLIS2's larger model (4B vs 500M) and higher polygon outputs do not compensate for the missing pose estimation stage.

### Pipeline Configuration

| Setting | TRELLIS1 | TRELLIS2 |
|:---|:---|:---|
